...
As explained in [output formats](../overview/outputs.md#facial-expressions), the variable `exp_global` contains 79 coefficients (presence levels of 79 deformations) per frame. These vectors can be used to compute [expressivity](expressivity.md) or [social dynamics](broken-reference) of facial expressions.&#x20;
