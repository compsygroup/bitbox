...
def visualize_and_export_can_land(
    ...
    """Video frames (top row) + 3D canonicalized landmarks (bottom row).

    - Uses pose to pick diverse frames if available, else evenly from land_can.
    - When overlays are provided: prefer cropping by rectangles when available; else crop by landmarks.
    - Overlay landmarks (blue) and rectangles (red). Blur toggle is available when any overlay is provided.
    """
    def _safe_df(d: Optional[dict]):
        return d["data"] if isinstance(d, dict) and "data" in d else None
...
def visualize_expressions_3d(
    ...
    if normalize in ("zscore", "minmax"):
        X = df[ge_cols].astype(float)
        if normalize == "zscore":
            mu, sd = X.mean(axis=0), X.std(axis=0).replace(0, 1.0)
            X = (X - mu) / sd
        else:
...
def write_video_overlay_html(
    ...
    """Write an HTML page showing a cropped video with overlays.

    Behavior:
      - Rectangles win over landmarks for cropping per frame.
      - Sticky crop: if a frame lacks data, reuse the last good crop.
      - 'Cover' scaling: cropped region fills the box (no borders).
      - Auto title if caller leaves default title.

    Auto title logic (only when title == 'Video Overlay'):
      - Only landmarks   -> "Video with Landmarks"
      - Only rectangles  -> "Video with Rectangles"
      - Both             -> "Video with Rectangle and Landmark Overlay"
    """
    # Optional fallback for non-portrait branch
    try:
    ...
    total_frame_keys = sorted(set(list(rects_map.keys()) + list(lands_map.keys())))
    ...
    if not use_chunks:
        if rects_map:
            rects_json = json.dumps(rects_map, separators=(",", ":"))
        if lands_map:
            lands_json = json.dumps(lands_map, separators=(",", ":"))
    else:
        def _make_chunks(src: dict, prefix: str) -> List[int]:
            if not src:
                return []
            starts: List[int] = []
            keys = sorted(set(int(k) for k in src.keys()))
            if not keys:
                return []
            min_f, max_f = keys[0], keys[-1]
            start = (min_f // CHUNK_SIZE) * CHUNK_SIZE
            while start <= max_f:
                ...
                if chunk:
                    ...
                    func = "registerRectsChunk" if prefix == "rects" else "registerLandsChunk"
    ...
    # Dynamic title (only override when caller left the default)
    if title == "Video Overlay":
        if has_lands and not has_rects:
...
<style>
  ...
  .container { display:flex; justify-content:center; padding:10px 16px 16px; }
  ...
  .toolbar { margin-top:32px; display:flex; gap:10px; justify-content:center; align-items:center; flex-wrap:nowrap; white-space:nowrap; }
  .btn { padding:6px 12px; border-radius:6px; border:2px solid #2ecc71; background:#f8f8f8; cursor:pointer; }
  .seek { width:220px; accent-color:#2ecc71; }
...
<script>
  ...
  const loadedRectChunks = new Set(), loadedLandChunks = new Set();
  const pendingRectChunks = new Set(), pendingLandChunks = new Set();
  ...
  function loadChunk(p,start){
    ...
    const starts = p==='rects'?RECT_CHUNK_STARTS:LAND_CHUNK_STARTS;
    const loaded = p==='rects'?loadedRectChunks:loadedLandChunks;
    const pending= p==='rects'?pendingRectChunks:pendingLandChunks;
  ...
  function ensureChunksForFrame(f){
    ...
    loadChunk('rects',st);loadChunk('lands',st);
    loadChunk('rects',st+CHUNK_SIZE);loadChunk('lands',st+CHUNK_SIZE);
    if(st-CHUNK_SIZE>=0){loadChunk('rects',st-CHUNK_SIZE);loadChunk('lands',st-CHUNK_SIZE);}
  }
  ...
  const HAS_RECTS=__HAS_RECTS__, HAS_LANDS=__HAS_LANDS__;
  ...
  function ensureAndGetDataFor(frame){
    ...
    return {lands:landsByFrame[frame]||null,rects:rectsByFrame[frame]||null};
  }

  function draw(){
    const dpr=window.devicePixelRatio||1;
    const targetW=view.width/dpr, targetH=view.height/dpr;
    const durFrames = (isFinite(vid.duration) && vid.duration>0 && FPS>0) ? Math.max(1, Math.floor(vid.duration*FPS)) : null;
    const rawFrame = Math.floor(vid.currentTime*FPS + 0.0001);
    ...
    // COVER scaling (no borders): fill BOX_W×BOX_H, crop excess
    ...
    try { vctx.drawImage(vid, sx, sy, sw, sh, dx, dy, drawW, drawH); }
    catch(e){
      if(lastCrop){
        const [lsx,lsy,lsw,lsh]=lastCrop;
        const s2 = Math.max(targetW/lsw, targetH/lsh);
        const dw = Math.ceil(lsw * s2), dh = Math.ceil(lsh * s2);
        const dx2 = Math.floor((targetW - dw) / 2), dy2 = Math.floor((targetH - dh) / 2);
        try { vctx.drawImage(vid, lsx, lsy, lsw, lsh, dx2, dy2, dw, dh); }
        catch(_){
          const sFull = Math.max(targetW/vw, targetH/vh);
          const dwF = Math.ceil(vw * sFull), dhF = Math.ceil(vh * sFull);
          const dxF = Math.floor((targetW - dwF) / 2), dyF = Math.floor((targetH - dhF) / 2);
          vctx.drawImage(vid, 0,0, vw, vh, dxF, dyF, dwF, dhF);
        }
      } else {
        const sFull = Math.max(targetW/vw, targetH/vh);
        const dw = Math.ceil(vw * sFull), dh = Math.ceil(vh * sFull);
        const dxF = Math.floor((targetW - dw) / 2), dyF = Math.floor((targetH - dh) / 2);
        vctx.drawImage(vid, 0,0, vw, vh, dxF, dyF, dw, dh);
      }
    }

    if(HAS_RECTS&&showRects&&rects){
      octx.strokeStyle='red'; octx.lineWidth=2; octx.globalAlpha=0.9;
      for(const r of rects){ octx.strokeRect((r.x-sx)*s+dx,(r.y-sy)*s+dy,r.w*s,r.h*s); }
    }
  ...
  window.addEventListener('keydown',e=>{
    ...
    else if(k==='r'&&HAS_RECTS){showRects=!showRects;updateBtns();}
    else if(k==='l'&&HAS_LANDS){showLands=!showLands;updateBtns();}
  });
  ...
  vid.addEventListener('ended',()=>{
    if(FPS>0 && isFinite(vid.duration)){
      try { vid.currentTime = Math.max(0, vid.duration - (1.0/FPS)); } catch(_){}
    }
    updateBtns();
  });
</script>
...
"""
    ...
    if fixed_portrait:
        html = _build_html(portrait_w, portrait_h)
    else:
        html = _build_html(TS_W, TS_H)
